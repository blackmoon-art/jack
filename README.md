...
端到端模拟电路设计系统：自然语言 → 模板匹配 → 参数计算 → SPICE 网表 → 仿真验证 → SVG 渲染。
...
│  三重仿真验证                                     │
│  ┌───────────────────────────────────────────┐  │
│  │ Layer 1: 语法门控 (_run_sim_check)         │  │
│  │  • ngspice -b 批量模式，15s 超时           │  │
│  │  • Apple Silicon subcircuit bug 自动检测   │  │
│  │  • E-source fallback (gain=100k)          │  │
│  │  ❌ 失败 → 返回错误，LLM 修正后重试         │  │
│  ├───────────────────────────────────────────┤  │
│  │ Layer 2: 电气门控 (_validate_characteristics)│ │
│  │  • AC 扫频 (1Hz→1MHz) 或 DC 工作点         │  │
│  │  • 放大器: 实测增益 vs 预期增益              │  │
│  │    - < 0.1% → "输出死区"                    │  │
│  │    - > 100x → "反馈开路"                    │  │
│  │  • 滤波器: 衰减量、-3dB cutoff 校验          │  │
│  │  • DC 死区检测 (< -150 dB)                  │  │
│  │  ❌ 失败 → 返回错误，LLM 修正后重试         │  │
│  ├───────────────────────────────────────────┤  │
│  │ Layer 3: 指标提取 (simulate_spice)         │  │
│  │  • .op: 节点电压、源电流、功耗              │  │
│  │  • .ac: DC增益、-3dB截止、相位、滚降率      │  │
│  │  • .tran: 上升时间、过冲、建立时间、纹波    │  │
│  │  ✅ 通过 → 结构化指标表 + 健康检查          │  │
│  └───────────────────────────────────────────┘  │
└─────────────────────────────────────────────────┘
        │ ✅ 通过
...
│     SPICE → 连接图 → BFS 分层布局 → Schemdraw    │
│     纯 SVG 元件符号 (暗色主题)，正交走线          │
│     fallback: 纯 Python BFS 布局 (无需 schemdraw) │
└─────────────────────────────────────────────────┘
```

### LLM 如何判断电路对错

LLM **不做主观判断** — 它通过调用工具链获取客观仿真反馈作为 ground truth：

```
LLM 生成/修改 SPICE 网表
         ↓
   调用 simulate_spice 或 draw_analog_svg
         ↓
   工具内部自动跑 ngspice 仿真
         ↓
   返回结构化结果给 LLM
...
| 层级 | 判断依据 | LLM 收到的反馈示例 |
|------|----------|-------------------|
| **语法错误** | ngspice 直接报错 | `Error on line 5: unknown component type` |
| **电气异常** | 实测值 vs 预期值比对 | `Gain severely low: 0.23x vs expected 10.0x — check Rf/R1 polarity` |
| **指标偏差** | AC/瞬态分析定量指标 | `Cutoff: 1023 Hz` vs 目标 `5kHz`，偏差 `-79.5%` |
| **健康警告** | 规则检查引擎 | `No AC signal at output (< -150 dB) — possible open circuit` |
| **闭环优化** | 迭代收敛判定 | `Target fc: 5.0kHz → Measured: 4.92kHz (off by 1.6%) ✅ Within spec` |

LLM 看到反馈后的行动逻辑：
1. 语法错误 → 修正拼写/格式
2. 电气异常 → 检查连接关系、元件值数量级
3. 指标偏差 → 按比例调整 R/C 值（`_adjust_params` 自动完成）
4. 健康警告 → 排查开路/短路/缺电源
...
| `design_circuit` | NL + specs (`fc=5kHz`) | 上述全流程 + **自动闭环优化** (比例调整, ≤5轮) |
...
| 类别 | 数量 | 模板 | 优化指标 |
|------|:---:|------|:---:|
| 滤波器 | 7 | RC 低通/高通、LC 低通、**Sallen-Key 有源低通/高通**、**MFB 带通**、**双T 陷波** | fc |
| 运放放大器 | 10 | 反相、同相、差分、求和反相、电压跟随器、**积分器**、**微分器**、**仪表放大器(3运放)**、**施密特触发器**、**比较器** | gain / fc |
| BJT 电路 | 6 | 共射放大、射极跟随器、电流镜、差分对、**共基放大**、**Cascode** | gain / Iref |
| MOSFET 电路 | 3 | 共源放大、**共漏(源跟随器)**、**MOSFET 差分对** | gain |
| 整流器 | 3 | 半波、全波桥式、**精密整流器(运放)** | — |
| 振荡器 | 2 | **Wien 桥**、**RC 相移** | fc |
| 分压器 | 1 | 分压器 | ratio |
...
模板存储在 `templates/circuits.yaml`（32 个内置）+ `templates/custom_circuits.yaml`（用户自定义）。
...
| v2 (当前) | SPICE→连接图→BFS 分层布局→Schemdraw | ✅ Rf 和运放并排同列 | **SVG** |

### 评分

| 维度 | 分 | 说明 |
|------|:--:|------|
| 正确性 | 8 | 模板计算正确、BFS 布局正确、BJT/MOSFET 模型完善。扣分：运放模型仍为行为级 |
| 鲁棒性 | 8 | 优雅降级、Apple Silicon bug 检测、优化阻尼限步、YAML fallback 硬编码 |
| 覆盖面 | 8.5 | **32 种模板**：运放+BJT+MOSFET+滤波器+整流+**振荡器**。扣分：缺噪声分析、缺变压器 |
| 架构 | 8.5 | YAML 模板分离、spice_common 模型共享、add_custom_template 热扩展、**三重验证门控** |
| 体验 | 8 | 中英文关键词、闭环设计、仿真门控、LLM 可保存模板、**结构化指标反馈** |
| 创新 | 8 | LLM + SPICE 闭环、仿真门控、LLM 读指标自主判断电路正确性 |
| **综合** | **8.2** | 从 17→32 模板，覆盖主要经典模拟电路类别 |

### 已知 Gap & Roadmap

| Gap | 说明 | 工作量 | 状态 |
|-----|------|:---:|:---:|
| ~~分立器件~~ | ~~BJT/MOSFET 模板 + 计算器 + 符号~~ | ~~2 天~~ | ✅ 已完成 |
| ~~可扩展性~~ | ~~YAML 模板 + LLM 保存 + 热加载~~ | ~~3-5 天~~ | ✅ 已完成 |
| ~~模板补全~~ | ~~有源高通/带通/陷波/积分器/仪表放大/振荡器等 15 个模板~~ | ~~2 天~~ | ✅ 已完成 (2026-07-03) |
| 多级电路自动组合 | `compose_circuit` 现需手动写 JSON，应支持 NL 描述 | ~1 周 | 🟡 中 |
...
## 代码架构审查 (2026-07-03 更新)

对 ~5500 行代码库的架构与代码质量审查。

### 总体评分

| 维度 | 评分 | 说明 |
|---|---|---|
| 架构设计 | ★★★★★ | 分层清晰，策略模式+注册表+依赖注入，Provider可插拔 |
| 代码质量 | ★★★★☆ | 干净整洁，模拟电路仿真闭环完整 |
| 可扩展性 | ★★★★★ | 新增策略/工具/Provider只需声明元数据 |
| 线程安全 | ★★★★★ | threading.local + 双检查锁 |
| LLM 调用效率 | ★★★★★ | 三层路由 85% 请求零 LLM 调用 |
| 测试覆盖 | ★★★☆☆ | 路由+策略测试好，电路模块缺单测 |
| 安全性 | ★★★★☆ | 沙箱路径校验 + shell=False |

### 已解决的 P0 问题

| # | 问题 | 解决方案 | 状态 |
|---|---|---|---|
| 1 | LLM Provider if-else分发 | `nano_agent/providers/` — BaseProvider ABC + AnthropicProvider + OpenAIProvider + ZhipuProvider | ✅ |
| 2 | 日志缺请求级追踪 | `logging_config.py` TraceFilter + `StrategyContext.trace_id` + `set_trace_id()` | ✅ |
| 3 | SPICE模型定义重复 | 提取 `tools/spice_common.py` — `_OPAMP_SUBCKT`, `_DIODE_MODEL` 共享 | ✅ |
| 4 | ngspice 缺少 timeout | `subprocess.run(timeout=15)` + TimeoutExpired 处理 | ✅ |

### 模拟电路：完整仿真闭环

```
draw_analog_svg (NL→模板→SPICE→ngspice验证→SVG)
  ├─ 11种模板: RC/LC/Sallen-Key低通, RC高通, 反相/同相/差分/求和放大, 半波/全波整流, 分压
  ├─ 中文关键词匹配 (keywords_cn)
  ├─ format_value 修复 (Meg→p遍历方向)
  ├─ SPICE生成: 自动算元件值, X前缀, opamp子电路注入
  ├─ 仿真门控: ngspice OP通过才出图, 失败返回错误供LLM修复
  └─ draw_analog_spice 同样带仿真门控 → LLM fix→retry→show闭环
  
design_circuit (新增): 端到端闭环 — 需求→设计→仿真→分析→迭代优化
```

### 画图路由 (37条规则)

```
generate_chart (20种): contour→bode→spectrum→waveform→regression→function→...
mermaid_chart  (8种):  时序图→状态机(FSM)→甘特图→思维导图→类图→ER图→流程图→架构图
draw_logic     (2条):  门级网表(AND/OR/DFF/MUX), 数字电路(ALU/寄存器/编码器)
draw_analog_svg(1条):  模拟电路(滤波器/运放/共射/整流/分压/比较器)
draw_block     (2条):  FIFO/OCC/DFT/PLL等IP → 框图; 系统框图/信号链/RF链
__classify__   (1条):  通用"电路" → LLM三分类(digital/analog/block) → 上述工具
ai_image       (1条):  照片/艺术/插画
create_ppt     (1条):  PPT/幻灯片
```
...
| 模块 | 行数 | 评分 | 变化 |
|---|---|---|---|
| `providers/` | ~520 | ★★★★★ | **新增**: BaseProvider ABC + 3个实现, llm.py 459→195 |
| `tools/analog_svg.py` | 1565 | ★★★★☆ | 899→1565: +仿真门控 +中文匹配 +电学验证 |
| `visual_router.py` | 426 | ★★★★★ | 390→426: +数字IP路由 +FSM关键词 +路由精确提示 |
| `agent.py` | 725 | ★★★★☆ | +电路审查 +trace_id +re模块级导入 |
| `llm.py` | 195 | ★★★★★ | 459→195: Provider委托, 仅保留重试+JSON |
| `tools/spice_simulator.py` | 1042 | ★★★★☆ | +timeout +spice_common模型共享 |
