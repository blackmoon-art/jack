...
_EXACT_ROUTES: list[tuple[str, str, dict]] = [
    ...
    # 折线（放后面，"线"字容易被其他词包含）
    # 只匹配明确的图表类型请求，不匹配泛化的"展示趋势/趋势分析"等 LLM 推理用语
    ...
    ("数字电路|逻辑门|门电路|digital.*circuit|logic.*gate|"
     ...
     "计数器|counter|分频器|"
     "全减器|全减|减法器|分配器|demux|优先编码|priority.*encoder|"
     "环形计数|ring.*counter|约翰逊|johnson.*counter|"
     "加减法|adder.*subtractor|数值比较|magnitude.*compar|"
...
_INTENT_PATTERNS: list[tuple[str, str, str, dict]] = [
    ...
    (r"变化|趋势|走势|增长|下降|波动|涨",
     r"温度|股价|数据|销量|指标|价格|人数|流量|收入|季度|月份",
     "generate_chart", {"chart_type": "line"}),
