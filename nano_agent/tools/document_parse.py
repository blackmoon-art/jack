...
class DocumentParser:
    ...
    def _libreoffice_to_pdf(self, filepath, out_path) -> str:
        ...
        # 估算超时：大文件给更多时间 (base 120s, max 600s)
        file_size_mb = filepath.stat().st_size / (1024 * 1024)
        timeout = max(120, min(600, int(file_size_mb * 30)))
    ...
    def _text_to_pdf(self, filepath, out_path) -> str:
        ...
        if truncated:
            story.append(Paragraph(
                f"<i>⚠ File has {total_lines} lines, showing first {MAX_LINES}.</i>",
                styles["Normal"]))
        ...
        if truncated:
            story.append(Paragraph(
                f"<i>... (truncated, {MAX_LINES}/{total_lines} lines shown)</i>",
                styles["Normal"]))
